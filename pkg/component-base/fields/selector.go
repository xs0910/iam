...
type Selector interface {
	...
	// requires a single specific field to be set, and if so returns the value it requires.
	...
	// If for a given requirement both field and value are transformed to empty string, the requirement is skipped.
	Transform(fn TransformFunc) (Selector, error)

	// Requirements converts this interface to Requirements to expose more detailed selection information.
...
func (t *hasTerm) Matches(fields Fields) bool {
	return fields.Get(t.field) == t.value
}
...
func (t *notHasTerm) Matches(fields Fields) bool {
	return fields.Get(t.field) != t.value
}
...
func (t andTerm) Matches(fields Fields) bool {
	for _, q := range t {
		if !q.Matches(fields) {
			return false
		}
...
// valueEscape prefixes \,= characters with a backslash.
var valueEscape = strings.NewReplacer(
	// escape \ characters
	`\`, `\\`,
